...
output_dir = "output/randomforest_output/rf_90d/rf_FS_90d_final"
model_filename = "FS_90d.joblib"
...
def gen_performance_metrics(y_true, y_preds, probabilities, model_name, round_to=3):
    acc = round(metrics.accuracy_score(y_true, y_preds), round_to)
    f1 = round(metrics.f1_score(y_true, y_preds), round_to)
    rec = round(metrics.recall_score(y_true, y_preds), round_to)
    auc = round(metrics.roc_auc_score(y_true, probabilities), round_to)
    tn, fp, fn, tp = metrics.confusion_matrix(y_true, y_preds).ravel()
    spec = round(tn / (tn + fp), round_to) if (tn + fp) > 0 else 0
    ppv = round(tp / (tp + fp), round_to) if (tp + fp) > 0 else 0
    npv = round(tn / (tn + fn), round_to) if (tn + fn) > 0 else 0

    return pd.DataFrame([[model_name, acc, f1, rec, spec, auc, ppv, npv]], 
                        columns=['Model', 'Accuracy', 'F1 Score', 'Recall', 'Specificity', 'ROC AUC Score', 'PPV', 'NPV'])
...
if __name__ == "__main__":
    ...
    metrics_df.to_csv(os.path.join(output_dir, 'metrics_rf_FS.csv'), index=False)
